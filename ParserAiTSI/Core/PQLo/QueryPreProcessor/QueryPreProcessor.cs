...
namespace Core.PQLo.QueryPreProcessor
{
	public class QueryPreProcessor : IQueryPreProcessor
	{
		...
		public string ProcessQuery()
		{
			...
			this.Fields = CreateFields(resultPart);
			CreatePQLTree(queryPart);
		...
		public void CreatePQLTree(List<string> elems)
		{
			...
			List<string> elements = SplitQuery(elems[0], queryMainTokens);
			if (this.queryParts.Count < 0) throw new Exception();
			foreach (var item in this.queryParts)
				switch (this.queryTypes.FirstOrDefault(qt => item.Contains(qt.ToLower())))
				{
					case "select": MakeSelectNode(item); break;
					case "such that": MakeSuchNode(item); break;
					case "with": MakeWithNode(item); break;
		...
		private void MakeWithNode(string item)
		{
			...
			{
				List<Field> attr = CreateWithNodeAttributes(elem.Trim());
		...
		private List<Field> CreateWithNodeAttributes(string withPart)
		{
			List<Field> attr = new List<Field>();
			int dotPos;
			if (this.matcher.WithHasTwoElem(withPart))
			{
				List<string> attrParts = withPart.Split('=').ToList();
		...
		private void FindPositions(string query, List<string> tokens)
		{
			...
			if (GetNextPosition(tokens, query)) _ = SplitQuery(query, tokens);
		}
		...
		private readonly List<PQLNode> selectNodes = new List<PQLNode>();
		private readonly List<PQLNode> suchNodes = new List<PQLNode>();
		private readonly List<PQLNode> withNodes = new List<PQLNode>();
		...
		private readonly string[] TokensList = {
			"assign", "stmtlst", "stmt", "while", "variable", "constant", "prog_line", "if", "call", "procedure"
		};

		private readonly string[] queryTypes =
		{
			"null",
			"select",
			"such that",
